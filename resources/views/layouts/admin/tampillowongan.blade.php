@include('layouts.component.header')
      ...
      <div class="page-header">
        <div class="row align-items-end">
          ...
          <div class="col-sm-auto">
            <a class="btn btn-primary" href="{{ route ('lowongan.tambah') }}">
              <i class="bi-person-plus-fill me-1"></i> Tambah Lowongan
            </a>
          </div>
          <!-- End Col -->		  
    ...
    <div class="row justify-content-between align-items-center flex-grow-1">
      <div class="col-12 col-md">
        <div class="d-flex justify-content-between align-items-center">
          <h5 class="card-header-title">Lowongan</h5>
        </div>
      </div>
  ...
  <div class="table-responsive datatable-custom">
    <table id="datatbleWithFilter" class="js-datatable table table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
      ...
      <td>{{$tampillowongan->nama_sekolah}}</td>
      <td>{{$tampillowongan->matapel}}</td>
      <td>{{$tampillowongan->kebutuhan}}</td>
      <td>
            <a href="{{ route ('lowongan.edit', $tampillowongan->id) }}" target="_blank" class="btn btn-white btn-sm">
                        <i class="bi-pencil-fill me-1"></i> Edit 
            </a>   
            <a href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#modalhapus{{ $tampillowongan->id }}" class="btn btn-white btn-sm">
                        <i class="bi-trash-fill me-1"></i> Hapus 
            </a>  			
			<form action="{{ route('lowongan.hapus', $tampillowongan->id) }}" method="post">
			@csrf
			@method('DELETE')
			<!-- Button trigger modal -->
			<!-- Modal -->
			<div class="modal fade" id="modalhapus{{ $tampillowongan->id }}"  tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
				<div class="modal-dialog">
				<div class="modal-content">
				<div class="modal-header">
				<h5 class="modal-title mt-0" id="myModalLabel">Hapus Data {{ $tampillowongan->nama_sekolah }}?</h5>
